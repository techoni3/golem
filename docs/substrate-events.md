...
Managed Codex App Server delivery uses the supervisor-owned canonical actor binding. Once the pinned
handshake, MCP status check, authenticated loopback typed `/brief` adapter, and idle thread are
valid, the health lease is `delivery_ready:true`. Dashboard dispatch sends a durable envelope to
that typed endpoint; acceptance is persisted before exactly one `turn/start`, and retries reuse the
saved mapping. Role activation and interrupt/halt remain visibly gated for managed Codex. This
changes only managed Codex delivery; CC and OpenCode receive the same active-message contract.
