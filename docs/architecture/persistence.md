...
TEXT project locations with a canonical path, optional raw observed path, and
only `main|worktree|registered|legacy` relation; ordinal, exact GOL-15
...
deterministic `COALESCE` uniqueness key, and use a composite
project/session/generation foreign key so a same-project generation cannot be
attached to the wrong logical session. Capability observations preserve the
separate GOL-26 qualification (`supported|experimental|unsupported|unknown`)
and are relationally bound to their endpoint readiness fact.
