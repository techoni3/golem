...
permitted only to `@golem/control-plane`, which is therefore the one
composition capability that can obtain a writer. The owner constructor, raw
SQLite/Kysely handles, schema table maps, and repositories are private modules.
...
canonical lifecycle generations; strongly scoped session aliases; separate
source/received/materialized/activity clocks and provenance; endpoint
revision/state/fence/delivery/readiness/control facts; and foreign-keyed
capability evidence. Terminal generations (`ended`, `errored`, `superseded`)
are history-only and excluded from `live_sessions`. Migration definitions are
