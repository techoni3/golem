...
cloud-model catalogue: managed Codex is OpenAI/GPT App Server; OpenCode local
is additive Ollama; Claude remains unqualified until a real consumption
journey exists.
...
direct/managed/push/pull/next-turn/app-server/readiness/control facts. It
starts no harness process and reads no user configuration.
...
`resolve.ts` composes immutable plans; `explain.ts` owns redacted failures and
canonical serialization; `types.ts` is the shared pure contract; and
`index.ts` is a thin public boundary.
