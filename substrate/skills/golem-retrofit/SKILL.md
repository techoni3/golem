...
1. Walk the tree (top-level + `.claude/` + `docs/` + `tracker/` + `journal/`).
...
4. Detect prior substrate presence:
   ...
   - Any tickets in `tracker/`? → record their states and titles so retrofit doesn't double-file.
...
3. Target is **populated** → leave alone. If its content is incompatible with substrate conventions (e.g. `tracker/` not using the canonical column directories, ADR-0001 with a different shape), record the incompatibility in the audit memo — do not auto-fix.
...
File discovery tickets in `tracker/triage/` for what retrofit observed. Use `labels: [retrofit, discovery]` so the CEO knows their origin.
...
- **Configuration drift** — anything retrofit could not auto-merge (settings.json populated; ADR-0001 conflicts; tracker has non-canonical columns).
...
If `tracker/` already had populated tickets before retrofit, do not file duplicates: check titles against the discovery list and skip overlaps.
...
**Tracker.** <count> discovery tickets in tracker/triage/ with labels [retrofit, discovery].
...
| `tracker/` exists with a few tickets | Preserve. Read titles; skip overlapping retrofit tickets. |
| `tracker/` exists with non-canonical columns (e.g. `todo/`, `wip/`) | File a single conflict ticket; let Tech Architect decide whether to migrate. Do not auto-rename. |
