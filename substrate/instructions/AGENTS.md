...
| `golem:browsing` | Any browser work — browsing, scraping, UI smoke, CDP/devtools, screenshots, anything behind a login. Load before launching a browser: one uniform Chrome path across harnesses, the shared-profile lock, and the headed login handoff all live there. |
| `golem:skill-authoring` | Writing or editing a skill, `SKILL.md`, agent persona, or this instructions file. Covers adaptive structure, front matter, ASD-STE100-inspired language, references, and editorial revision. |
