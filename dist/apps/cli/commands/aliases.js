...
function render(shell, names) {
    const targetFor = (name) => name === "golem-codex"
        ? "codex"
        : name === "golem-opencode"
            ? "opencode"
            : name === "golem-claude"
...
export async function runAliases(input, io) {
    ...
    if (!names) {
        io.stderr("cli.usage: alias names must be non-native names (never codex, claude, opencode, or golem)");
        return 2;
    }
    ...
    if (action === "uninstall") {
        if (!input.apply) {
            ...
            if (input.json)
                ...
                io.stdout(`dry-run remove ${target}; re-run with --apply to remove only this Golem-owned file`);
            return 0;
        }
        ...
        if (input.json)
            ...
            io.stdout(`removed ${target}; no shell rc file was changed`);
        return 0;
    }
    if (action !== "install") {
    ...
    if (input.json)
        ...
        io.stdout(`wrote ${target}; source it explicitly (your shell configuration was not edited)`);
    return 0;
}
