...
function golem(pi) {
  ...
  function persistDeadLetter(root, source, name, code) {
    try {
      const dir = path.join(root, "dead-letter");
      fs.mkdirSync(dir, { recursive: true, mode: 448 });
      const key = reference(name);
      const target = path.join(dir, `${key}.${crypto.randomUUID()}.json`);
      ...
      try {
        fs.unlinkSync(path.join(root, "published", name));
      } catch {
      }
      try {
        fs.unlinkSync(path.join(root, "processing", `${name}.lease.json`));
      } catch {
      }
      try {
        fs.unlinkSync(path.join(root, "acks", `${name}.lease.json`));
      } catch {
      }
    } catch {
    }
  ...
  function bindingFor(rawSessionId) {
    const key = safeSessionKey(rawSessionId);
    if (!key) return null;
    try {
  ...
  function diagnostic(rawSessionId, code) {
    ...
    try {
      const dir = path.join(home, "pi-adapter", "diagnostics");
      fs.mkdirSync(dir, { recursive: true, mode: 448 });
      const sessionReference = reference(rawSessionId);
      const target = path.join(dir, `${sessionReference}.json`);
  ...
  function spoolSignal(signal) {
    ...
    fs.mkdirSync(pending, { recursive: true, mode: 448 });
    const target = path.join(pending, `${signal.event_id}.json`);
    const tmp = path.join(pending, `.${signal.event_id}.${process.pid}.tmp`);
  ...
  function lifecycle(ctx, nativeEvent, observations = {}) {
    ...
    const binding = bindingFor(rawSessionId);
    if (!binding) {
      diagnostic(rawSessionId, "pi.binding.unqualified");
      return;
    }
    const observedAt = (/* @__PURE__ */ new Date()).toISOString();
    const generation = { project_id: binding.project_id, session_id: binding.session_id, generation_id: binding.generation_id };
    const signal = (event_kind, payload) => {
      ...
      spoolSignal({
        ...
        event_kind,
        producer: "golem-pi-extension",
        producer_instance_id: binding.producer_instance_id,
        harness: "pi",
        producer_sequence: producerSequence,
        correlation_id: binding.generation_id,
        deduplication_key: `pi:${binding.generation_id}:${producerSequence}:${event_kind}`,
        owner_fence: binding.owner_fence,
        clocks: { source_observed_at: observedAt, received_at: observedAt },
        provenance: { source: "adapter", evidence_id: binding.generation_id, confidence: "observed" },
        clear_fields: [],
        payload
      });
    };
  ...
  function record(ctx, event, status, observations = {}) {
    ...
    try {
      upsertSessionFact({
        ...
        continuation_key: id,
        harness: "pi",
        locator: { raw_session_id: id, session_file: ctx.sessionManager.getSessionFile() },
        project_path: ctx.cwd,
        name: safeEventLabel(ctx.sessionManager.getSessionName() || ""),
        status,
        delivery: { mode: "next_turn", push: false },
    ...
    try {
      lifecycle(ctx, event, observations);
    } catch {
      diagnostic(id, "pi.runtime_signal.spool_failed");
    }
  }
    ...
    for (const { root, name, canonical, claimToken } of pendingPickup) {
      try {
        ...
        if (canonical && claimToken) {
          ...
          const ack = path.join(root, "acks", `${name}.lease.json`);
          const temporary = `${ack}.${process.pid}.tmp`;
          fs.writeFileSync(temporary, JSON.stringify({ acknowledgement_id: acknowledgementId, claim_token: claimToken }) + "\n", { mode: 384 });
          fs.renameSync(temporary, ack);
        }
      } catch {
      }
    }
  ...
  pi.on("input", (_event, ctx) => {
    ...
    const canonicalRoot = binding ? path.join(home, "pi-next-turn", binding.session_id, binding.generation_id) : null;
    const messages = [];
    ...
    for (const item of work) {
      ...
      const processing = path.join(root, "processing", name);
      try {
        fs.mkdirSync(path.dirname(processing), { recursive: true });
        if (!item.claimed) fs.renameSync(source, processing);
        const value = JSON.parse(fs.readFileSync(processing, "utf8"));
        if (typeof value?.text !== "string") throw new Error("invalid text");
        messages.push(value);
        pendingPickup.push({ root, name });
      } catch {
        persistDeadLetter(root, processing, name, "pi.next_turn.record_invalid");
      }
    }
    if (canonicalRoot && binding) {
      const canonicalProcessing = path.join(canonicalRoot, "processing");
      const canonicalPending = path.join(canonicalRoot, "pending");
      const canonicalWork = [
      ...
      for (const item of canonicalWork) {
        const { name, source } = item;
        const processing = path.join(canonicalRoot, "processing", name);
        try {
          fs.mkdirSync(path.dirname(processing), { recursive: true });
          if (!item.claimed) fs.renameSync(source, processing);
          const value = JSON.parse(fs.readFileSync(processing, "utf8"));
          ...
          messages.push(value);
          pendingPickup.push({ root: canonicalRoot, name, canonical: true, claimToken: value.claimToken });
        } catch {
          persistDeadLetter(canonicalRoot, processing, name, "pi.next_turn.record_invalid");
        }
      }
    }
