...
function launcherCommand(role, name, profile = null, harness = 'pi', sessionId = null) {
...
export async function spawnWorker({ role, name = null, project = null, cwd = process.cwd(), profile = null } = {}) {
  ...
  try {
    ...
    newSession({
      ...
      cwd: projectInfo.projectRoot,
      command: launcherCommand(roleRecord.name, worker.name, profileOverride, worker.preset?.harness || 'pi', canonicalSessionId),
      width: 200,
      height: 50,
