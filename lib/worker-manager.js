 ...
 */
export async function spawnWorker({ role, name = null, project = null, cwd = process.cwd(), profile = null } = {}) {
  ...
  try {
    ...
    newSession({
      ...
      cwd: projectInfo.projectRoot,
      command: launcherCommand(roleRecord.name, worker.name, profileOverride),
      width: 200,
      height: 50,
...
export async function peekSessionTerminal(sessionId, { lines = 100, projectId = null } = {}) {
  ...
  const worker = workers.find((w) => w.session_id === sessionId || w.name === sessionId);
  if (worker) {
    try {
  ...
  // Fallback: check project socket or default socket by session name
  const candidateSocket = projectId ? socketForProject(projectId) : null;
  for (const socket of [candidateSocket, null].filter((v, i, a) => a.indexOf(v) === i)) {
    if (socket !== undefined && hasSession(sessionId, { socket })) {
      try {
        const text = capturePane(sessionId, lines, { socket });
        return {
          ok: true,
          ...
          lines,
          text,
          attach_hint: `golem attach ${sessionId}${projectId ? ` --project ${projectId}` : ''}`,
          updated_at: new Date().toISOString(),
        };
      } catch (err) {
        return {
