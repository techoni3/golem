...
function launcherCommand(role, name) {
  const configured = process.env.GOLEM_WORKER_CLI;
  const args = ['pi', '--role', role, '--name', name];
  if (configured) {
    // Tests and local source launches provide a JavaScript CLI path. An
    // installed render may only have the `golem` executable on PATH, so allow
    // the same override to name a command rather than forcing process.execPath
    // to parse a non-JavaScript launcher.
    return fs.existsSync(configured)
      ? [process.execPath, configured, ...args]
      : [configured, ...args];
  }
  if (fs.existsSync(DEFAULT_CLI)) return [process.execPath, DEFAULT_CLI, ...args];
  return [process.env.GOLEM_BIN || 'golem', ...args];
}
...
function callerWorkerSessionId() {
  const pgid = processGroupId(process.pid);
  if (!pgid) return null;
  const candidates = listWorkers().filter((row) => (
    row.session_id
    && ['spawning', 'live', 'failed'].includes(String(row.state || '').toLowerCase())
    && Number(row.pid) === pgid
  ));
  const match = candidates.find((row) => processGroupMatches(row.pid, { name: row.name }));
  return match?.session_id ?? null;
}

export async function killWorker(name, options = {}) {
  ...
  const hasCallerOverride = Object.prototype.hasOwnProperty.call(options, 'callerId');
  const callerId = hasCallerOverride
    ? options.callerId
    : (callerSessionId() ?? callerWorkerSessionId());
