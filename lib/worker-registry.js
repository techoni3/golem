...
import { resolveSocket, socketForProject } from './tmux-driver.js';
...
/** The socket a row's tmux session lives on: stored value, else the legacy shared 'golem' server. */
function rowSocket(row) {
  return resolveSocket(row?.tmux_socket ?? null);
}

function nextName(role, projectId, workers) {
  const socket = resolveSocket(socketForProject(projectId));
  for (let index = 1; ; index += 1) {
    const candidate = `${role}${index}`;
    if (!nameCollides(candidate, projectId, socket, workers)) return candidate;
  }
}

/**
 * A candidate worker name collides when an occupied row already holds it in
 * the SAME project — regardless of socket, so a legacy 'golem' row and a new
 * per-project row in one project can never share a name that findWorker and
 * killWorker would be unable to tell apart — or on the SAME resolved socket,
 * because tmux session ids are unique per server and another project's worker
 * on that socket would still collide at the tmux identifier.
 */
function nameCollides(candidate, projectId, socket, workers) {
  return workers.some((row) => (
    row.name === candidate
    && occupied(row)
    && (row.project_id === projectId || rowSocket(row) === socket)
  ));
}
...
export function claimWorker({
  ...
  return withRegistryLock(file, () => {
    const registry = readRegistry(file);
    // Resolve before claiming so the name check and the stored row agree on
    // the swarm the worker will actually run on.
    const socket = resolveSocket(socketForProject(projectId.trim()));
    const workerName = name == null ? nextName(role.trim(), projectId, registry.workers) : normalizeName(name);
    const collision = registry.workers.find((row) => (
      row.name === workerName && occupied(row)
      && (row.project_id === projectId.trim() || rowSocket(row) === socket)
    ));
    if (collision) {
      throw new Error(`worker name already exists: ${workerName} (project ${collision.project_id ?? 'unknown'}, socket ${rowSocket(collision)})`);
    }
