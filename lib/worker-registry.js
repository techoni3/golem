...
function nextName(role, projectId, workers) {
  ...
  for (let index = 1; ; index += 1) {
    ...
    // tmux session names are unique per socket, not per machine: each project
    // owns its swarm's socket, so sequences restart per project. Legacy rows
    // on the shared 'golem' socket still collide with each other, which the
    // per-socket comparison keeps protecting.
    const collision = workers.some((row) => row.name === candidate && occupied(row) && rowSocket(row) === socket);
...
export function claimWorker({
  ...
  return withRegistryLock(file, () => {
    ...
    const collision = registry.workers.find((row) => row.name === workerName && occupied(row) && rowSocket(row) === socket);
    if (collision) {
