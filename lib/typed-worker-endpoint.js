...
export function normalizeTypedWorkerInbox(inbox = {}, { historyLimit = DEFAULT_TYPED_DELIVERY_HISTORY_LIMIT } = {}) {
  ...
  // `deliveries` is a bounded inspection/history view. Replay identity is a
  // distinct durable index and is deliberately never evicted: an old accepted
  // envelope must remain a duplicate forever, even after its display row ages
  // out of the recent history window.
  const replay_index = {};
  if (inbox.replay_index && typeof inbox.replay_index === 'object' && !Array.isArray(inbox.replay_index)) {
    for (const [envelopeId, delivery] of Object.entries(inbox.replay_index)) {
      if (delivery && typeof delivery === 'object' && typeof delivery.envelope_id === 'string') {
        replay_index[envelopeId] = normalizeDelivery(delivery);
      }
    }
  }
  ...
  for (const delivery of deliveries) {
    if (typeof delivery.envelope_id === 'string' && !replay_index[delivery.envelope_id]) {
      replay_index[delivery.envelope_id] = delivery;
    }
  }
  const recent = deliveries.map((delivery) => replay_index[delivery.envelope_id] || delivery);
  return {
    schema: Math.max(4, Number(inbox.schema) || 0),
...
export function getTypedDelivery(inbox, envelopeId) {
  return inbox?.replay_index?.[envelopeId] ?? inbox?.deliveries?.find((delivery) => delivery.envelope_id === envelopeId) ?? null;
}
...
function replaceTypedDelivery(inbox, envelopeId, patch) {
  ...
  inbox.replay_index[envelopeId] = next;
...
export function claimTypedDelivery(inbox, envelope, { at = Date.now(), historyLimit = DEFAULT_TYPED_DELIVERY_HISTORY_LIMIT } = {}) {
  ...
  inbox.replay_index[delivery.envelope_id] = delivery;
...
export function releaseTypedDeliveryClaim(inbox, envelopeId, { attemptId = null, error = null } = {}) {
  ...
  delete inbox.replay_index[envelopeId];
