  ...
  for (const abs of listFilesRecursive(skillsDir)) {
    ...
    items.push({
      ...
      build: () => {
        ...
        return isSkillDoc
          ? htmlComment(path.join('skills', rel)) + applyTemplate(text, HARNESS_CONTEXT)
          : text;
      },
    });
  }
...
export function computeConfigText(currentText, { mcpGolem, skillsPath }) {
  let next = currentText;
  next = applyEdits(next, modify(next, ['mcp', 'golem'], mcpGolem, FMT));
  const parsed = parse(next) ?? {};
  const existing = Array.isArray(parsed.skills?.paths) ? parsed.skills.paths : [];
  const merged = existing.includes(skillsPath) ? existing : [...existing, skillsPath];
  next = applyEdits(next, modify(next, ['skills', 'paths'], merged, FMT));
  return next;
}
