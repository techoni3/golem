...
// Role cards render alongside skills under the opencode render dir.
//
// Nothing reads this directory today, and that is worth stating plainly rather
// than implying otherwise. An opencode session gets its card from two places
// that both bypass this render: `tracker-context.sh` resolves
// `$SCRIPT_DIR/../roles`, which the shim points at `<repo>/substrate/roles`; and
// `readRoleCard()` resolves the overlay, then `$GOLEM_ROLES_DIR`, then the
// packaged card next to whichever copy of `session-role.js` is loaded.
//
// It renders anyway for parity — so a role card removed from substrate is pruned
// everywhere rather than only on two harnesses — and so `GOLEM_ROLES_DIR` has a
// stable, sync-managed directory to point at for an install without the repo.
...
/**
 * RenderItems for role cards, output as `<role>.md` into rolesOutDir(). Copied
 * verbatim: cards are a handful of lines, carry no harness-specific dialect, and
 * are read as plain text rather than parsed.
 * @param {object} ctx
 * @param {string} ctx.substrateRoot
 */
...
export function buildInstructionPlan({ substrateRoot }) {
  ...
  if (fs.existsSync(abs)) items.push({
    ...
    outputRelPath: path.join('opencode', 'AGENTS.md'),
    sourceSha256: sha256File(abs),
    type: 'block',
  ...
  if (fs.existsSync(trackerContextAbs)) items.push({
    ...
    outputRelPath: path.join('opencode', 'tracker-context.md'),
