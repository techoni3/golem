...
export function buildPlan({ repoRoot, substrateRoot: suppliedSubstrateRoot, packageVersion }) {
  ...
  const readme = '# Golem for Pi (Tier A worker)\n\nRequires `@earendil-works/pi-coding-agent@0.80.10` and Node.js >=22.19. Launch local Ollama with `golem pi --provider ollama --model <local-id>` or direct cloud with `golem pi --provider ollama-cloud --model <cloud-id>`.\n\nThe managed launcher syncs this canonical render, stores profile and session data below `GOLEM_HOME`, supports `--resume <native-session-id>` plus native arguments after `--`, and installs the pinned `@ifi/pi-provider-ollama@0.5.1` package into that private profile on first use. Local `ollama/...` and direct `ollama-cloud/...` models coexist in the same `/model` picker; non-Ollama custom providers retain the selective source `models.json` bridge. The extension registers shared Golem tools, injects Golem-owned instructions and builder/explorer/reviewer role context at safe turn boundaries, exposes progressive skills, renders bounded project context, and provides authenticated typed-worker delivery. Pre-acceptance failures remain replayable; accepted work interrupted by a crash is outcome-unknown and requires an explicit correlated recovery/redispatch. Pi extensions execute with the user\'s full host authority: project trust is not a sandbox. Lead/standalone orchestration, Pi-native subagents, and bundled browser/LSP features are deferred. The launcher never mutates the user\'s normal Pi profile.\n';
  const runtime = [
