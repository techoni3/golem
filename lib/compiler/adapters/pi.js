...
export function buildPlan({ repoRoot, packageVersion }) {
  ...
  const readme = '# Golem for Pi (Tier B)\n\nRequires Pi and Node.js >=22.19. Load without changing a profile:\n\n```sh\npi -e "$GOLEM_HOME/renders/pi/golem.ts"\n```\n\nThe extension records canonical session facts and pulls durable addressed inbox entries on the next user input. It does not claim idle push delivery.\n';
  return [item('extension', 'golem.ts', extension), ...[
