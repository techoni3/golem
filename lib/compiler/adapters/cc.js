...
export function buildPlan({ substrateRoot, repoRoot, packageVersion }) {
  ...
  for (const name of ['session-role.js', 'golem-home.js', 'legacy-writer-guard.js', 'session-facts.js', 'session-facts-write.js']) {
...
export function syncMcpChannelDeps({ repoRoot, outDir }) {
  ...
  // C4 activates the relocatable typed MCP artifact. The old channel closure is
  // retained only for the documented rollback window and is never referenced
  // by the generated .mcp.json.
  let artifactCopied = false;
  if (fs.existsSync(artifact)) {
    // Keep the active artifact adjacent to the rollback-only channel directory.
