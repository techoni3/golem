...
//   .mcp.json, .claude-plugin/plugin.json, README.md, and the typed MCP artifact.
//
...
/**
 * Copy the single relocatable typed MCP artifact into the render.
 * No render or package install owns a nested npm install.
 */
export function syncMcpChannelDeps({ repoRoot, outDir }) {
  const staleChannel = path.join(outDir, 'mcp', 'channel');
  const artifact = path.join(repoRoot, 'packages', 'mcp-adapter', 'dist', 'golem-mcp.mjs');
  fs.rmSync(staleChannel, { recursive: true, force: true });
  ...
  return { copied: false, artifactCopied, dest: staleChannel };
}
