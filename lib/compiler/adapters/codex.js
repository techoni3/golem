...
export function buildPlan({ substrateRoot, repoRoot, packageVersion }) {
  ...
  const lifecycle = path.join(repoRoot, 'shims', 'codex', 'direct-lifecycle.mjs');
  out.push(item('hook:direct-lifecycle.mjs', `${root}/hooks/direct-lifecycle.mjs`, lifecycle));
  for (const rel of ['golem-home.js', 'legacy-writer-guard.js', 'project-id.js', 'session-facts.js', 'session-registry.js', 'session-role.js']) {
    const abs = path.join(repoRoot, 'lib', rel);
    out.push(item(`lib:${rel}`, `${root}/lib/${rel}`, abs));
  }
  const mcpArtifact = path.join(repoRoot, 'packages', 'mcp-adapter', 'dist', 'golem-mcp.mjs');
  out.push(item('mcp:golem-mcp.mjs', `${root}/mcp/golem-mcp.mjs`, mcpArtifact));
  const manifest = {
  ...
  const mcp = { golem: { command: 'node', args: ['mcp/golem-mcp.mjs'], cwd: '.' } };
  const capabilities = {
    schema: 1, harness: 'codex', tier: 'B', lifecycle: true, mcp: true, subagents: true,
    delivery: ['pull'], push_delivery: false, control: false, discovery: true,
    readiness: 'pull_only', delivery_qualification: 'unproven',
