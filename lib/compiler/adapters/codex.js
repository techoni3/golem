...
export function buildPlan({ substrateRoot, repoRoot, packageVersion }) {
  ...
  const trackerContext = path.join(substrateRoot, 'hooks', 'tracker-context.sh');
  if (fs.existsSync(trackerContext)) {
    out.push(item('hook:tracker-context.sh', `${root}/hooks/tracker-context.sh`, trackerContext));
  }
