...
export function buildPlan({ substrateRoot, repoRoot, packageVersion }) {
  ...
  for (const rel of ['index.js', 'package.json', 'package-lock.json']) {
    const abs = path.join(repoRoot, 'mcp', 'channel', rel);
  ...
  const capabilities = {
    ...
    delivery: ['pull', 'next_turn'], push_delivery: false,
    limitation: 'Ordinary Codex CLI exposes no documented out-of-band turn injection API; queued work is accepted only when a later hook/turn observes it.',
  };
