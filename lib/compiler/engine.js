...
export function render({ target, outDir, items, packageVersion, force = false }) {
  ...
  const prevFiles = lock.targets?.[key]?.files ?? {};
  const newFiles = {};
  const written = [];
  const unchanged = [];
  const tampered = [];
  ...
  for (const item of items) {
    ...
    if (diskSha === outputSha) {
      ...
      newFiles[item.key] = { source_sha256: item.sourceSha256, output_path: item.outputRelPath, output_sha256: outputSha };
      continue;
    }
 ...
 */
export function checkDrift({ target, outDir, items }) {
  const lock = readLockfile();
  const prevFiles = lock.targets?.[lockKey(target, outDir)]?.files ?? {};
  const drifted = [];
  const seen = new Set();
  ...
  for (const item of items) {
    ...
    const diskSha = currentOutputHash(outAbs);
    if (!prev) {
      drifted.push({ key: item.key, reason: 'new' });
      continue;
    }
