...
export class PiNativeAdapter {
  ...
  input(event, ctx) {
    ...
    } else if (this.activeDeferredInput && event.source === 'extension'
      && JSON.stringify(event.text) === JSON.stringify(typeof this.activeDeferredInput.content === 'string'
  ...
  async agentStart(_event, ctx) {
    ...
    if (!pending && this.activeDeferredInput?.inputSeen) {
