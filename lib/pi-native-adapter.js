...
export class PiNativeAdapter {
  ...
  async acceptEnvelope(envelope) {
    ...
    const timer = setTimeout(() => {
      ...
      if (current?.lifecycle_state === 'claimed') {
        ...
        this.persistLease();
        resolveAcceptance({ ok: false, accepted: false, http_status: 503, error: 'Pi did not accept the typed turn before timeout' });
      }
  ...
  async control(envelope, kind) {
    ...
    // A claimed turn has not crossed Pi's acceptance boundary and is safe to
    // release. Its waiting publisher receives an explicit retryable result.
    if (this.pendingAcceptance) {
      const pending = this.pendingAcceptance;
      clearTimeout(pending.timer);
      this.pendingAcceptance = null;
    ...
    if (!nativeBusy && active?.lifecycle_state !== 'accepted') {
