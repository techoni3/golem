...
const contracts = [
  ...
  {
    name: 'session_spawn',
    description: 'Spawn one detached Pi worker through the shared worker manager. Returns the live worker name, session id, role, model, and attach hint. Unknown roles return GOLEM_UNKNOWN_ROLE; readiness timeouts return GOLEM_WORKER_TIMEOUT; both are structured, actionable errors. A failed spawn remains inspectable with the CLI.',
    inputSchema: object({
      role: string('Pi worker role: builder|explorer|reviewer.'),
      project: string('Optional project path, registry id, name, or contract project_id. Defaults to the caller project.'),
      name: string('Optional worker/tmux/session name. Auto-named when omitted.'),
    }, ['role']),
  },
  {
    name: 'session_kill',
    description: 'Kill one detached worker through the shared worker manager. Refuses to kill the authenticated caller session, then verifies the worker process group is empty before returning.',
    inputSchema: object({ name: string('Worker name from session_spawn or sessions_dispatchable.') }, ['name']),
  },
  {
