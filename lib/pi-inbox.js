...
export function enqueuePiBrief(canonicalId, text, metadata = {}, options = {}) {
  ...
  const messageId = crypto.randomUUID(); const final = path.join(pending, `${messageId}.json`); const tmp = path.join(root, `.${messageId}.${process.pid}.tmp`);
...
export function claimPiPickupAcks({ home = golemHome() } = {}) {
  ...
  for (const session of fs.existsSync(base) ? fs.readdirSync(base) : []) {
    ...
    for (const name of fs.existsSync(dir) ? fs.readdirSync(dir).filter((value) => value.endsWith('.json')) : []) {
      const file = path.join(dir, name); const claim = `${file}.claim.${process.pid}`;
      try { fs.renameSync(file, claim); out.push({ file: claim, value: JSON.parse(fs.readFileSync(claim, 'utf8')) }); } catch {}
    }
  }
  return out;
}
