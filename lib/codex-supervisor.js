...
export class CodexSupervisor extends EventEmitter {
  ...
  handleNotification(message) {
    ...
    if (index < 0 && inbox.in_flight_envelope_id) {
      index = inbox.deliveries.findIndex((delivery) => delivery.envelope_id === inbox.in_flight_envelope_id);
    }
    if (index >= 0) {
  ...
  async acceptDeliverySerial(envelope) {
    ...
    const latestIndex = latest.deliveries.findIndex((delivery) => delivery.envelope_id === claimed.envelope_id);
    ...
    if (latestIndex < 0) throw new Error(`claimed envelope ${claimed.envelope_id} disappeared from the durable inbox`);
