...
import { codexSupervisorsJsonPath, dashboardJsonPath, typedDeliveryTombstonesDbPath } from './golem-home.js';
import { projectIdFor, resolveProjectRoot } from './project-id.js';
import {
  acceptTypedDelivery,
  claimTypedDelivery,
  closeTypedWorkerEndpoint as closeServer,
  getTypedDelivery,
  interruptTypedDelivery,
  normalizeTypedWorkerInbox as normalizeInbox,
...
import { readTypedDeliveryTombstone, upsertTypedDeliveryTombstone } from './typed-delivery-tombstones.js';
import {
...
function dashboardBaseUrl(env = process.env) {
  if (env.GOLEM_DASHBOARD_URL) return env.GOLEM_DASHBOARD_URL.replace(/\/+$/, '');
  try {
    const dashboard = JSON.parse(fs.readFileSync(dashboardJsonPath(), 'utf8'));
    if (typeof dashboard?.url === 'string' && dashboard.url.trim()) return dashboard.url.replace(/\/+$/, '');
    if (dashboard?.host && dashboard?.port) return `http://${dashboard.host}:${dashboard.port}`;
  } catch { /* canonical local fallback below */ }
  return 'http://dashboard.golem.localhost:7420';
}

// A received JSON-RPC error proves that the native server rejected this exact
// request before it accepted a turn. It is categorically different from an
// exit, timeout, or broken transport, where native acceptance remains unknown.
export class CodexRpcServerRejection extends Error {
  constructor(method, payload) {
    super(`${method} rejected: ${JSON.stringify(payload)}`);
    this.name = 'CodexRpcServerRejection';
    this.method = method;
...
async function listenForHealth(supervisor) {
  return startTypedWorkerEndpoint({
    canonicalId: supervisor.canonicalId,
    ownerToken: supervisor.ownerToken,
    kind: 'codex-supervisor',
    deliveryReady: () => supervisor.deliveryReady(),
    acceptDelivery: (envelope) => supervisor.acceptDelivery(envelope),
    onRequest: async ({ request, url, readBody, sendJson, sameSecret }) => {
      ...
      if (url.pathname !== '/approvals' && !approvalDetailMatch && !approvalDecisionMatch) return false;
      ...
      if (target !== supervisor.canonicalId || !sameSecret(owner, supervisor.ownerToken)) {
        sendJson(403, { ok: false, error: 'operator approval authentication failed' });
        return true;
      }
      if (url.pathname === '/approvals' && request.method === 'GET') {
        sendJson(200, { canonical_id: supervisor.canonicalId, pending: supervisor.listPendingApprovals() });
        return true;
      }
      if (approvalDetailMatch && request.method === 'GET') {
        const detail = supervisor.approvalDetail(approvalDetailMatch[1]);
        sendJson(detail ? 200 : 404, detail
          ? { canonical_id: supervisor.canonicalId, approval: detail }
          : { ok: false, error: 'approval is not pending in this live supervisor' });
        return true;
      }
      if (approvalDecisionMatch && request.method === 'POST') {
        let body;
        try { body = JSON.parse(await readBody()); } catch (error) {
          sendJson(400, { ok: false, error: error.message || 'invalid JSON approval decision' });
          return true;
        }
        try {
          sendJson(200, { ok: true, approval: supervisor.decideApproval(approvalDecisionMatch[1], body?.decision) });
        } catch (error) {
          sendJson(/not pending/.test(String(error?.message)) ? 404 : 400, { ok: false, error: error.message || String(error) });
        }
        return true;
      }
      sendJson(405, { ok: false, error: 'unsupported operator approval operation' });
      return true;
    },
  });
}
...
export class CodexAppServerRpc extends EventEmitter {
  ...
  receive(line) {
    ...
    if (Object.hasOwn(message, 'id')) {
      ...
      if (message.error) pending.reject(new CodexRpcServerRejection(pending.method, message.error));
...
export class CodexSupervisor extends EventEmitter {
  constructor({ canonicalId = `codex-${crypto.randomUUID()}`, cwd = process.cwd(), command = 'codex', env = process.env, registryFile = codexSupervisorsJsonPath(), tombstoneFile = typedDeliveryTombstonesDbPath(), mode = 'headless' } = {}) {
    super();
    ...
    this.reportedTerminalDeliveries = new Set();
    this.lifecycleReportPromise = null;
  }
  ...
  rememberTypedDelivery(delivery) {
    if (!delivery || ![
      'accepted', 'settled', 'interrupted', 'recovery_required',
    ].includes(delivery.lifecycle_state)) return null;
    return upsertTypedDeliveryTombstone(this.canonicalId, delivery, { file: this.tombstoneFile });
  }

  async reportTerminalDelivery(delivery) {
    if (!delivery?.envelope_id || !TERMINAL_TYPED_DELIVERY_STATES.has(delivery.lifecycle_state)) return false;
    if (this.reportedTerminalDeliveries.has(delivery.envelope_id)) return true;
    const record = this.readRecord();
    const ownerToken = record?.health?.owner_token || this.ownerToken;
    if (!ownerToken) return false;
    try {
      const reported = await reportTypedDeliveryLifecycle({
        baseUrl: dashboardBaseUrl(this.env),
        canonicalId: this.canonicalId,
        ownerToken,
        delivery,
      });
      if (!reported) return false;
      this.reportedTerminalDeliveries.add(delivery.envelope_id);
      return true;
    } catch {
      return false;
    }
  ...
  async flushTerminalLifecycleReports() {
    if (this.lifecycleReportPromise) return this.lifecycleReportPromise;
    this.lifecycleReportPromise = (async () => {
      const inbox = normalizeInbox(this.readRecord()?.inbox);
      for (const delivery of inbox.deliveries) {
        if (TERMINAL_TYPED_DELIVERY_STATES.has(delivery.lifecycle_state)) {
          await this.reportTerminalDelivery(delivery);
        }
      }
    })().finally(() => { this.lifecycleReportPromise = null; });
    return this.lifecycleReportPromise;
  }

  migrateTypedDeliveryTombstones(rawInbox) {
    // Schema 5 writes a compact tombstone at the same acceptance boundary as
    // the registry update. Once tracker-owned terminal evidence retires that
    // tombstone, re-reading stale schema-5 JSON must not recreate it on every
    // supervisor restart. Only pre-tombstone schemas need this migration.
    if ((Number(rawInbox?.schema) || 0) >= 5) return;
    const rows = [
      ...(Array.isArray(rawInbox?.deliveries) ? rawInbox.deliveries : []),
      ...(rawInbox?.replay_index && typeof rawInbox.replay_index === 'object'
        ? Object.values(rawInbox.replay_index) : []),
    ];
    const seen = new Set();
    for (const row of rows) {
      if (!row?.envelope_id || seen.has(row.envelope_id)) continue;
      seen.add(row.envelope_id);
      const delivery = normalizeInbox({ deliveries: [row] }).deliveries[0];
      this.rememberTypedDelivery(delivery);
    }
  }

  // A completion notification can arrive before the corresponding turn/start
  // response (or before that response is lost). The persistent tombstone is
  // authoritative over an older claimed registry row. Keep useful original
  // metadata where it exists, but never let it regress the terminal state.
  reconcileTerminalTypedDelivery(inbox, envelopeId) {
    const local = getTypedDelivery(inbox, envelopeId);
    const tombstone = readTypedDeliveryTombstone(this.canonicalId, envelopeId, { file: this.tombstoneFile });
    if (!tombstone || !TERMINAL_TYPED_DELIVERY_STATES.has(tombstone.lifecycle_state)) {
      return { inbox, delivery: local, reconciled: false };
    }
    const state = tombstone.lifecycle_state === 'settled'
      ? 'completed'
      : tombstone.lifecycle_state === 'interrupted'
        ? 'failed'
        : 'recovery_pending';
    const delivery = {
      ...(local ?? {}),
      ...
      attempt_id: tombstone.accepted_attempt_id ?? local?.attempt_id ?? null,
      accepted_attempt_id: tombstone.accepted_attempt_id ?? local?.accepted_attempt_id ?? null,
      lifecycle_state: tombstone.lifecycle_state,
      state,
      turn_id: tombstone.turn_id ?? local?.turn_id ?? null,
      claimed_at: tombstone.claimed_at ?? local?.claimed_at ?? null,
      accepted_at: tombstone.accepted_at ?? local?.accepted_at ?? null,
      settled_at: tombstone.settled_at ?? local?.settled_at ?? null,
      interrupted_at: tombstone.interrupted_at ?? local?.interrupted_at ?? null,
      recovery_required_at: tombstone.recovery_required_at ?? local?.recovery_required_at ?? null,
      expires_at: tombstone.expires_at ?? local?.expires_at ?? null,
    };
    const index = inbox.deliveries.findIndex((row) => row.envelope_id === envelopeId);
    if (index >= 0) inbox.deliveries[index] = delivery;
    else inbox.deliveries = [...inbox.deliveries, delivery].slice(-DELIVERY_HISTORY_LIMIT);
    if (inbox.in_flight_envelope_id === envelopeId) inbox.in_flight_envelope_id = null;
    return { inbox, delivery, reconciled: true };
  }

  reconcileTerminalTypedDeliveries(inbox) {
    for (const delivery of [...inbox.deliveries]) {
      this.reconcileTerminalTypedDelivery(inbox, delivery.envelope_id);
    }
    return inbox;
  }

  reconcileDeliveryStartAmbiguity(claimed, error) {
    const latest = this.readRecord();
    const inbox = normalizeInbox(latest?.inbox);
    const reconciled = this.reconcileTerminalTypedDelivery(inbox, claimed.envelope_id);
    const delivery = reconciled.delivery ?? getTypedDelivery(inbox, claimed.envelope_id);
    if (delivery && TERMINAL_TYPED_DELIVERY_STATES.has(delivery.lifecycle_state)) {
      const turnState = delivery.lifecycle_state === 'settled'
        ? 'idle'
        : delivery.lifecycle_state === 'recovery_required'
          ? 'recovery_pending'
          : 'failed';
      this.updateRecord({
        turn: {
          ...(latest?.turn ?? {}),
          state: turnState,
          turn_id: delivery.turn_id ?? latest?.turn?.turn_id ?? null,
      ...
      this.writeRuntimeFact('supervisor-delivery-start-response-reconciled', {
        envelope_id: delivery.envelope_id,
        delivery_state: delivery.lifecycle_state,
      });
      return { delivery, terminal: true };
    }
    const recovery = requireTypedDeliveryRecovery(inbox, claimed.envelope_id, { error });
    this.rememberTypedDelivery(recovery.delivery);
    this.updateRecord({
      turn: { state: 'recovery_pending', turn_id: null, envelope_id: claimed.envelope_id, failed_at: iso(), kind: 'dispatch' },
      inbox,
      last_error: `delivery turn start outcome unknown: ${error}`,
    });
    if (this.healthAddress) this.persistLease(this.healthAddress);
    this.writeFact('error', 'supervisor-delivery-recovery-pending', { envelope_id: claimed.envelope_id });
    return { delivery: recovery.delivery, terminal: false };
  }
  ...
  writeFact(status, lifecycleEvent, observations = {}) {
    ...
    const terminal = ['dead', 'stopped', 'failed'].includes(String(status || '').toLowerCase());
    const fact = upsertSessionFact({
      ...
      model_provider: this.modelProvider,
      status,
      ...(terminal ? { ended_at: new Date().toISOString() } : { ended_at: null }),
      waiting_for: status === 'waiting' ? (observations.waiting_for ?? null) : null,
      capabilities: {
        typed_worker: true,
        typed_worker_protocol: 1,
      },
    ...
    // Retire the ordinary-hook raw twin so it cannot linger as a second card.
    if (terminal && this.threadId && this.threadId !== this.canonicalId) {
      try { markSessionFactsEnded([this.threadId], { status: 'superseded' }); } catch { /* best-effort */ }
      try { markSessionsEnded([this.threadId], { status: 'superseded' }); } catch { /* best-effort */ }
    }
    return fact;
  }
  ...
  startLeaseHeartbeat(host, port) {
    ...
    this.leaseHeartbeat = setInterval(() => {
      try {
        this.persistLease({ host, port });
        void this.flushTerminalLifecycleReports();
      } catch (error) { this.handleUnexpectedExit({ error, reason: 'lease_renewal_failed' }); }
    }, interval);
  ...
  handleNotification(message) {
    ...
    const record = this.readRecord();
    const inbox = normalizeInbox(record?.inbox);
    let delivery = inbox.deliveries.find((candidate) => candidate.turn_id === turn.id) ?? null;
    ...
    // is safe to attach that completion to its claimed mapping.
    if (!delivery && inbox.in_flight_envelope_id) {
      delivery = getTypedDelivery(inbox, inbox.in_flight_envelope_id);
    }
    if (delivery) {
      // App Server can complete before the turn/start response persists. The
      // shared graph deliberately forbids claimed -> terminal, so correlate
      // the implied native start before recording this completion.
      if (delivery.lifecycle_state === 'claimed') {
        acceptTypedDelivery(inbox, delivery.envelope_id, { turnId: turn.id ?? null });
        delivery = getTypedDelivery(inbox, delivery.envelope_id);
      }
      if (completed) settleTypedDelivery(inbox, delivery.envelope_id, {
        turnId: turn.id ?? delivery.turn_id ?? null,
        completionStatus: turn.status ?? null,
      });
      else interruptTypedDelivery(inbox, delivery.envelope_id, {
        turnId: turn.id ?? delivery.turn_id ?? null,
        completionStatus: turn.status ?? null,
      });
      this.rememberTypedDelivery(getTypedDelivery(inbox, delivery.envelope_id));
    }
    ...
    const terminalState = delivery && !completed ? 'failed' : 'idle';
    this.updateRecord({
  ...
  deliveryResult(delivery, { duplicate = false, httpStatus = 200, attemptId = null } = {}) {
    return typedDeliveryResult(delivery, { duplicate, httpStatus, attemptId });
  }
  ...
  async acceptDeliverySerial(envelope) {
    ...
    const claim = claimTypedDelivery(inbox, envelope, {
      historyLimit: DELIVERY_HISTORY_LIMIT,
      lookupTombstone: (envelopeId) => readTypedDeliveryTombstone(this.canonicalId, envelopeId, { file: this.tombstoneFile }),
    });
    if (claim.fenced) {
      return {
        ok: false,
    ...
    if (claim.duplicate) {
      ...
      return this.deliveryResult(claim.delivery, { duplicate: true, attemptId: envelope.attempt_id });
    }
    if (claim.busy || !this.deliveryReady(record)) {
      return { ok: false, accepted: false, http_status: 409, error: 'managed Codex target is not idle and delivery-ready' };
    }

    // The shared lifecycle writes the synchronous claim before the external
    // RPC. The adapter exposes acceptance only after the native turn start is
    // correlated; a lost start response becomes recovery-required, never an
    // automatic replay.
    const claimed = claim.delivery;
    this.updateRecord({
      turn: { state: 'starting', turn_id: null, envelope_id: claimed.envelope_id, started_at: claimed.claimed_at, kind: 'dispatch' },
      inbox,
    });
    if (this.healthAddress) this.persistLease(this.healthAddress);
    this.writeFact('busy', 'supervisor-delivery-claimed', { envelope_id: claimed.envelope_id });
    ...
    try {
      const turnStart = {
        threadId: this.threadId,
        cwd: this.cwd,
        clientUserMessageId: claimed.envelope_id,
      ...
      if (error instanceof CodexRpcServerRejection) {
        // The server explicitly refused before turn creation. Release only
        // this matching claimed attempt: it is safely replayable and must not
        // be misrepresented as an accepted/recovery-required side effect.
        const released = releaseTypedDeliveryClaim(inbox, claimed.envelope_id, {
          attemptId: claimed.attempt_id,
          error: error.message,
        });
        this.updateRecord({
          thread_status: { type: 'idle' },
          turn: { state: 'idle', turn_id: null, envelope_id: null, failed_at: iso(), kind: 'dispatch' },
          inbox,
          last_error: `delivery turn start rejected: ${error.message}`,
        });
        if (this.healthAddress) this.persistLease(this.healthAddress);
        this.writeRuntimeFact('supervisor-delivery-rejected-before-accept', {
      ...
      // even when its response was lost. Re-read durable state first: a
      // notification may already have accepted and settled this exact turn.
      const outcome = this.reconcileDeliveryStartAmbiguity(claimed, error.message);
      if (outcome.terminal) return this.deliveryResult(outcome.delivery, { httpStatus: 200, attemptId: envelope.attempt_id });
      return { ...this.deliveryResult(outcome.delivery, { httpStatus: 503, attemptId: envelope.attempt_id }), ok: false, error: 'delivery outcome is recovery-pending; it will not be replayed automatically' };
    }
    const turnId = started?.turn?.id;
    if (!turnId) {
      const outcome = this.reconcileDeliveryStartAmbiguity(claimed, `turn/start returned no turn id: ${JSON.stringify(started)}`);
      if (outcome.terminal) return this.deliveryResult(outcome.delivery, { httpStatus: 200, attemptId: envelope.attempt_id });
      return { ...this.deliveryResult(outcome.delivery, { httpStatus: 503, attemptId: envelope.attempt_id }), ok: false, error: 'delivery outcome is recovery-pending; it will not be replayed automatically' };
    }
    const latest = normalizeInbox(this.readRecord()?.inbox);
    const latestDelivery = getTypedDelivery(latest, claimed.envelope_id);
    ...
    if (!latestDelivery) throw new Error(`claimed envelope ${claimed.envelope_id} disappeared from the durable inbox`);
    const { delivery: persisted } = acceptTypedDelivery(latest, claimed.envelope_id, { turnId });
    this.rememberTypedDelivery(persisted);
    this.updateRecord({
      turn: ['completed', 'failed', 'recovery_pending'].includes(persisted.state)
        ? this.readRecord()?.turn
        : { state: 'busy', turn_id: turnId, envelope_id: claimed.envelope_id, started_at: iso(), kind: 'dispatch' },
      inbox: latest,
    });
    return this.deliveryResult(persisted, { httpStatus: 202, attemptId: envelope.attempt_id });
  }

  handleUnexpectedExit({ error, reason }) {
    if (this.expectedExit || this.stopped) return;
    this.stopped = true;
    void this.finalizeUnexpectedExit({ error, reason });
  }

  async finalizeUnexpectedExit({ error, reason }) {
    ...
    const record = this.readRecord();
    const inbox = normalizeInbox(record?.inbox);
    const inFlight = getTypedDelivery(inbox, inbox.in_flight_envelope_id);
    if (inFlight?.lifecycle_state === 'accepted') {
      const recovery = requireTypedDeliveryRecovery(inbox, inFlight.envelope_id, {
    ...
    this.updateRecord({
      ...
      turn: {
        ...(this.readRecord()?.turn ?? {}),
        state: inFlight?.lifecycle_state === 'accepted' ? 'recovery_pending' : 'failed',
        failed_at: iso(),
      },
      inbox,
    ...
    await this.flushTerminalLifecycleReports().catch(() => {});
  ...
  async start() {
    ...
    this.migrateTypedDeliveryTombstones(existing?.inbox);
    const recoveredInbox = normalizeInbox(existing?.inbox);
    this.reconcileTerminalTypedDeliveries(recoveredInbox);
    ...
    try {
      ...
      const resumedInbox = normalizeInbox(this.readRecord()?.inbox);
      const inFlight = resumedInbox.deliveries.find((delivery) => (
        delivery.envelope_id === resumedInbox.in_flight_envelope_id
      ));
      if (this.mode === 'headless' && lifecycle.resumed && inFlight) {
        requireTypedDeliveryRecovery(resumedInbox, inFlight.envelope_id, { error: 'supervisor resumed with an unsettled typed delivery' });
        this.updateRecord({
          turn: {
