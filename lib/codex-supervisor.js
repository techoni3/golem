...
import { assertLegacyWriterAllowed, legacyWriterAuthority } from './legacy-writer-guard.js';
...
const GOLEM_MCP_LEGACY_ENTRYPOINT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'mcp', 'channel', 'index.js');
const GOLEM_MCP_TYPED_ENTRYPOINT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'packages', 'mcp-adapter', 'dist', 'golem-mcp.mjs');
...
export function managedCodexMcpConfig({ canonicalId, cwd }) {
  ...
  return [
    ...
    `mcp_servers.${GOLEM_MCP_SERVER_NAME}.args=[${tomlString(legacyWriterAuthority().stage === 'C4' ? GOLEM_MCP_TYPED_ENTRYPOINT : GOLEM_MCP_LEGACY_ENTRYPOINT)}]`,
...
/**
 * Compatibility composition for callers migrating off the JSON supervisor
 * registry. The canonical managed adapter is loaded lazily so legacy CLI/help
 * remains usable before workspace packages have been built.
 */
export async function createManagedCodexSupervisor(options) {
  const module = await import('@golem/adapter-codex');
  return new module.ManagedCodexSupervisor(options);
}

function updateRegistry(canonicalId, mutate, { file = codexSupervisorsJsonPath() } = {}) {
  assertLegacyWriterAllowed('codex-supervisors.json');
