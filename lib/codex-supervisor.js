...
// Pinned 0.144.4 App Server request/response pairs. "approve" always maps to
...
export class CodexAppServerRpc extends EventEmitter {
  ...
  denyServerRequest(message) {
    // Responses are taken from the pinned 0.144.4 schema. Unknown requests
...
export class CodexSupervisor extends EventEmitter {
  ...
  async initializeRpc(existing) {
    ...
    if (existing?.thread_id) {
      ...
      this.updateRecord({ thread_name: this.threadName, thread_status: resumed?.thread?.status ?? null });
      return { resumed: true };
    }
    ...
    this.updateRecord({ thread_name: this.threadName, thread_status: started?.thread?.status ?? null });
