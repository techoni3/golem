...
export class CodexSupervisor extends EventEmitter {
  ...
  deliveryResult(delivery, { duplicate = false, httpStatus = 200 } = {}) {
    return typedDeliveryResult(delivery, { duplicate, httpStatus });
  }
  ...
  async acceptDeliverySerial(envelope) {
    const record = this.readRecord();
    const inbox = normalizeInbox(record?.inbox);
    const claim = claimTypedDelivery(inbox, envelope, { historyLimit: DELIVERY_HISTORY_LIMIT });
    if (claim.duplicate) {
      ...
      return this.deliveryResult(claim.delivery, { duplicate: true });
    }
    ...
    try {
      ...
      return { ...this.deliveryResult(recovery.delivery, { httpStatus: 503 }), ok: false, error: 'delivery outcome is recovery-pending; it will not be replayed automatically' };
    }
    const turnId = started?.turn?.id;
    if (!turnId) {
    ...
    return this.deliveryResult(persisted, { httpStatus: 202 });
  }
