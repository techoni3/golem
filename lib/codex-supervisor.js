...
import { pruneTypedDeliveryTombstones, readTypedDeliveryTombstone, upsertTypedDeliveryTombstone } from './typed-delivery-tombstones.js';
import {
...
export class CodexSupervisor extends EventEmitter {
  ...
  rememberTypedDelivery(delivery) {
    ...
    pruneTypedDeliveryTombstones({ file: this.tombstoneFile });
  ...
  async acceptDeliverySerial(envelope) {
    ...
    if (claim.fenced) {
      return { ok: false, accepted: false, http_status: 409, error: 'legacy typed replay history is fenced; request a freshly issued envelope' };
    }
    if (claim.duplicate) {
    ...
    try {
      ...
      const recovery = requireTypedDeliveryRecovery(inbox, claimed.envelope_id, { error: error.message });
      ...
      return { ...this.deliveryResult(recovery.delivery, { httpStatus: 503, attemptId: envelope.attempt_id }), ok: false, error: 'delivery outcome is recovery-pending; it will not be replayed automatically' };
    }
    const turnId = started?.turn?.id;
    if (!turnId) {
      const recovery = requireTypedDeliveryRecovery(inbox, claimed.envelope_id, { error: `turn/start returned no turn id: ${JSON.stringify(started)}` });
      this.rememberTypedDelivery(recovery.delivery);
      this.updateRecord({
        turn: { state: 'recovery_pending', turn_id: null, envelope_id: claimed.envelope_id, failed_at: iso(), kind: 'dispatch' },
        inbox,
