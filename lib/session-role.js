...
export function readRoleRegistry() {
  if (roleRegistryCache) return roleRegistryCache.map((r) => ({ ...r, ...(Object.hasOwn(r, 'exec') ? { exec: cloneRoleExec(r.exec) } : {}) }));
  ...
  if (raw) {
    ...
    for (const builtin of BUILTIN_ROLES) {
      ...
      if (!existing) {
        ...
        // An index written before execution presets existed is still valid. Add
        // the builtin seed only when the user has not supplied an exec block.
...
export function updateRoleMeta(name, patch = {}) {
  ...
  roles[idx] = normalizeRoleMeta(next);
  writeRolesIndex(roles);
  return roles[idx];
}
