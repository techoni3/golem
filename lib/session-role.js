...
function roleRegistryStateFor(roles) {
  ...
  for (const role of roles) {
    if (Object.hasOwn(role, 'exec')) known_exec[role.name] = cloneRoleExec(role.exec);
  }
  return {
...
function roleRegistryLossError(names) {
  ...
  return new Error(
    ...
    + `Restore ${rolesIndexPath()} from a known-good copy; last-known presets are recorded in ${roleRegistryStatePath()}`,
  );
}
...
function assertRoleRegistryExecIntact(raw, state) {
  if (!state) {
    if (raw?.version >= ROLE_REGISTRY_VERSION) {
      throw new Error(`role registry provenance is missing at ${roleRegistryStatePath()}; refusing to trust execution presets`);
    }
    return;
  }
...
export function readRoleRegistry() {
  ...
  const state = readRoleRegistryState();
...
export function updateRoleMeta(name, patch = {}) {
  ...
  if (Object.hasOwn(patch, 'exec')) {
    if (Object.hasOwn(current, 'exec') && patch.exec == null) {
      throw new Error(`cannot remove execution preset for "${normalized}"; provide a valid preset instead`);
    }
