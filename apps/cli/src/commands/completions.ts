...
function shellFor(input: CompletionCommandInput): CompletionShell | undefined {
...
function words(registry: readonly CliCommandDefinition[]): string {
...
export function renderCompletion(
	...
	const optionWords = [
		...new Set(
			publicCommands(registry).flatMap((command) =>
				(command.options ?? []).flatMap((option) =>
					option.flags.split(" ").filter((word) => word.startsWith("-")),
				),
			),
		),
	]
	...
	return `# golem completion — generated from typed registry\ncomplete -c golem -f\n${publicCommands(
		...
		.map(
			(command) =>
				`complete -c golem -a '${command.name}' -d '${command.summary.replaceAll("'", "\\'")}'`,
		)
		.join("\n")}\n`;
}

export function completionFile(shell: CompletionShell): string {
	return launcherOwnedPath(join("completions", `golem.${shell}`));
}

export async function runCompletions(
	input: CompletionCommandInput,
	io: CompletionCommandIo,
	registry: readonly CliCommandDefinition[],
): Promise<number> {
