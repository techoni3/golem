...
export function launcherConfigPath(scope: ConfigScope): string {
	if (scope === "project")
		return join(
			resolve(process.env.GOLEM_PROJECT_ROOT ?? process.cwd()),
			".golem",
			"launcher.jsonc",
		);
	return resolve(
...
export function launcherOwnedPath(name: string): string {
	return join(launcherHome(), name);
}

export const filesystemConfigPort: ConfigTextPort = {
	async readText(path) {
...
export function readOwnedJson(path: string): unknown {
...
export function writeOwnedJson(path: string, value: unknown): void {
	mkdirSync(dirname(path), { recursive: true });
