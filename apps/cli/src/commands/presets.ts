import type { DeliveryMode, Harness } from "@golem/contracts";
import {
	type Backend,
	type ConfigScope,
	type LaunchPreset,
...
const harnesses = new Set<Harness>(["codex", "opencode", "claude"]);
...
const secretLike =
	/(?:api[_-]?key|token|secret|password|credential|authorization)\s*=/iu;

function safePresetText(value: string): boolean {
	return (
		value.trim().length > 0 &&
		!/[\0\r\n]/u.test(value) &&
...
function scopeFor(value: string | undefined): ConfigScope | undefined {
...
function publicPreset(preset: LaunchPreset): Record<string, unknown> {
	return {
		name: preset.name,
...
function history(): {
	...
	const source = value as { favorites?: unknown; recent?: unknown };
...
export function recordRecentPreset(name: string): void {
	if (!/^[a-z0-9][a-z0-9._-]{0,127}$/iu.test(name)) return;
	const current = history();
	writeHistory({
		favorites: current.favorites,
		recent: [name, ...current.recent.filter((entry) => entry !== name)].slice(
			0,
			12,
		),
	});
...
function error(io: PresetCommandIo, message: string): number {
...
export async function runPresets(
	input: PresetCommandInput,
	io: PresetCommandIo,
): Promise<number> {
	const [action = "list", name] = input.positionals;
	const scope = scopeFor(input.scope);
	if (!scope) return error(io, "--scope must be user or project");
	...
	if (action === "list") {
		...
		if ("code" in merged) return error(io, merged.message);
		...
		if (!input.json)
			for (const capability of listed.capabilities.filter(
				(entry) => !entry.launchable,
			))
				io.stdout(
					`${capability.id}: unavailable (${capability.qualification}); remedy: ${capability.launch.remediation}`,
				);
		return 0;
	}
	if (!name) return error(io, `presets ${action} requires a preset name`);
	if (action === "favorite") {
		const merged = mergeLauncherConfig({ [scope]: document });
		const exists =
			config.presets.some((preset) => preset.name === name) ||
			(!("code" in merged) &&
				merged.presets.some((preset) => preset.name === name));
		if (!exists) return error(io, `preset ${name} is not configured`);
		const current = history();
	...
	let next: readonly LaunchPreset[];
	if (action === "remove") {
		...
		const harness = input.positionals[2] as Harness | undefined;
		...
		if (!input.backend || !backends.has(input.backend as Backend))
			return error(
				io,
		...
		if (!input.delivery || !deliveryModes.has(input.delivery as DeliveryMode))
			return error(io, "presets set requires --delivery <mode>");
		const preset: LaunchPreset = {
			name,
			harness,
			backend: input.backend as Backend,
			model_selector: input.model,
			delivery_mode: input.delivery as DeliveryMode,
			native_args: [],
			env_key_refs: [],
		};
		next = [
			...config.presets.filter(
				(entry) => !(entry.name === name && entry.harness === harness),
			),
			preset,
		];
	} else {
	...
	if (input.json) io.stdout(stableCliJson({ ...result, apply: true }));
