...
`.claude/settings.json` wires only the journaling hooks here — no lint / git-guardrails, since the CEO's job at this level is orchestration, not source-code editing. Per-project `.claude/settings.json` files inside `golem-projects/<name>/` carry their own stack-aware lint and guardrail wiring.
