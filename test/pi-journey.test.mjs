...
try {
  ...
  const executable = path.join(temp, 'golem.mjs'); fs.copyFileSync(path.join(root, 'golem.ts'), executable);
  ...
  assert.match(handlers.input({ text: 'next turn' }, ctx).text, /queued brief/); assert.equal(fs.existsSync(path.join(inboxDir, 'pi-session-uuid.jsonl')), false);
