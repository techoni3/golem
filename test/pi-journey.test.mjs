...
try {
  ...
  const q1Ack = fs.readdirSync(path.join(inboxDir, 'acks')).find((x) => x.startsWith('q1.')); fs.renameSync(path.join(inboxDir, 'acks', q1Ack), path.join(inboxDir, 'acks', `${q1Ack}.claim.stale`));
  fs.writeFileSync(path.join(inboxDir, 'acks', 'malformed.json'), '{');
  await drainer.tick(); assert.equal(commentsDelivered, false); assert.ok(fs.readdirSync(path.join(inboxDir, 'acks')).some((x) => x.includes('.claim.'))); assert.ok(fs.existsSync(path.join(inboxDir, 'acks', 'malformed')));
