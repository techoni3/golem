...
try {
  ...
  const sourceModels = `${JSON.stringify({ providers: {
    ollama: {
      ...
      models: [{ id: 'deepseek-v4-flash:0731-cloud', input: ['text'] }, { id: 'must-not-copy' }],
    },
    unrelated: { baseUrl: 'http://127.0.0.1:9999/v1', api: 'openai-completions', apiKey: 'private', models: [{ id: 'must-not-copy' }] },
  } }, null, 2)}\n`;
...
`, { mode: 0o700 });
  ...
  assert.equal(readFileSync(path.join(sourcePiProfile, 'models.json'), 'utf8'), sourceModels, 'unsupported installed Pi never mutates the normal profile');
